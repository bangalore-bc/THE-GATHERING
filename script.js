...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (locContainer && data.locations) {
        ...
        data.locations.forEach((loc, index) => {
            ...
            if (isMobile) {
                el.className = `relative group overflow-hidden p-5 flex flex-col justify-end border border-stone-200/80 cursor-pointer rounded-2xl card-lift @container`;
                el.style.minHeight = '220px';
            } else {
                el.className = `absolute group overflow-hidden p-8 flex flex-col justify-end border border-stone-200/80 cursor-pointer rounded-2xl card-lift @container`;
            ...
            el.innerHTML = `
                ...
                <div class="relative z-10 flex flex-row justify-between items-end w-full gap-4 mt-auto">
                    <div class="flex flex-col text-left flex-1">
                        ...
                        <h3 class="font-display ${index === 0 ? 'font-extrabold' : 'font-bold'} uppercase tracking-tighter ${textColor} leading-[1.1] m-0 text-[clamp(1.5rem,calc(1rem_+_3cqw),4.5rem)]">${loc.name}</h3>
                    </div>
